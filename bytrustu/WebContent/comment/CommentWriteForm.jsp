...
<script type="text/javascript">
	...
	function writeCmt(bNum, precId, cGroup, cStep, cIndent) {
		...
		if (!cContent) {
			...
			var param = "bNum=" + bNum + "&precId=" + precId  + "&cContent=" + cContent + "&cGroup=" + cGroup + "&cStep=" + cStep + "&cIndent=" + cIndent;
			httpRequest = getXMLHttpRequest();
			httpRequest.onreadystatechange = checkFunc;
			httpRequest.open("POST", "CommentReplyWriteAction.co", true);
	...
	<form name="frm">
		...
		<button type="button" onclick="writeCmt('<%=bNum%>','<%=precId%>','<%=cGroup%>','<%=cStep%>','<%=cIndent%>')">작성</Button>
		</div>
